...
export class ProcessedEventsService {
  ...
  async buildRetryPayload(id: string): Promise<BillingEventPayload> {
    ...
    return {
      ...
      deviceId: this.readOptionalString(event.payload, 'deviceId'),
      deviceName: this.readOptionalString(event.payload, 'deviceName'),
      platform: this.readOptionalString(event.payload, 'platform'),
      installId: this.readOptionalString(event.payload, 'installId'),
      email,
